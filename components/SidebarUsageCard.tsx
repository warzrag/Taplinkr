...
function UsageRow({ icon: Icon, label, item }: { icon: typeof Link2; label: string; item: UsageItem }) {
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="sidebar-usage-progress h-0.5 overflow-hidden rounded-full bg-white/[0.07]">
        <div
...
export default function SidebarUsageCard() {
  ...
  return (
    <section className="sidebar-usage-card mx-0.5 mt-2 rounded-xl border border-violet-500/15 bg-gradient-to-b from-violet-500/[0.08] to-transparent p-2.5" aria-label="Plan usage">
      ...
      <div className="sidebar-usage-grid grid grid-cols-2 gap-x-3 gap-y-2">
        ...
        <div className="sidebar-usage-social col-span-2 flex items-center gap-2 border-t border-white/[0.06] pt-2 text-[10px]">
      ...
      {usage.plan === 'free' && (
        usage.canManagePlan ? (
          <button
            ...
            className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-lg border border-violet-500/20 bg-violet-500/10 px-2 py-1.5 text-[10px] font-semibold text-violet-200 transition hover:bg-violet-500/15"
          >
            Upgrade plan <ExternalLink className="h-3 w-3" />
          </button>
        ) : (
          <p className="mt-2 border-t border-white/[0.06] pt-2 text-center text-[10px] leading-4 text-[#77778a]">
