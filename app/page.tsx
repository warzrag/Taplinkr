...
const benefits = [
  {
    ...
    description: 'Customize colors, visuals, links, and calls to action without touching code.',
  },
  {
    icon: MousePointerClick,
    title: 'Send every click to the right place',
    description: 'Guide each visitor to the right offer with clear links and deep links.',
  },
  {
    icon: BarChart3,
    title: 'Results you can understand',
    description: 'Track views, clicks, sources, countries, and devices from a clear dashboard.',
  },
]
...
export default function Home() {
  ...
  const checkUsername = useMemo(() => debounce(async (value: string) => {
    ...
    try {
      ...
      setError('Availability check is temporarily unavailable')
    } finally {
      setChecking(false)
    }
  }, 400), [])
    ...
    <div className="min-h-screen overflow-x-hidden bg-white text-neutral-950 dark:bg-neutral-950 dark:text-white">
      ...
      <main>
          ...
          <Container className="relative grid min-h-[calc(100svh-4rem)] gap-12 py-16 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:py-24">
            <div>
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm text-white/80">
                <Sparkles className="h-4 w-4 text-cyan-300" /> Your online presence, simplified
              </div>
              <h1 className="max-w-3xl text-4xl font-semibold leading-[1.03] tracking-tight sm:text-6xl">
                One link. More clicks that convert.
              </h1>
              <p className="mt-6 max-w-2xl text-base leading-7 text-white/70 sm:text-lg">
                Create a polished page for your content, offers, and social profiles. Share it anywhere and understand what your audience cares about.
              </p>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Link href="/demo" className="inline-flex h-12 items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/5 px-6 text-sm font-semibold text-white transition hover:bg-white/10">
                  <Eye className="h-4 w-4" /> View a live example
                </Link>
              </div>
              <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm text-white/60">
                <span className="inline-flex items-center gap-2"><Check className="h-4 w-4 text-emerald-300" /> Free, no credit card required</span>
                <span className="inline-flex items-center gap-2"><Check className="h-4 w-4 text-emerald-300" /> Go live in minutes</span>
              </div>
            </div>
        ...
        <section className="border-b border-neutral-200 bg-neutral-50 py-8 dark:border-white/10 dark:bg-white/[.03]">
          <Container>
            <div className="mx-auto max-w-3xl">
              <label htmlFor="username" className="mb-3 block text-center text-sm font-semibold">Choose your Taplinkr address</label>
              <div className="flex flex-col gap-3 sm:flex-row">
                <div className="flex min-w-0 flex-1 items-center rounded-xl border border-neutral-300 bg-white px-4 focus-within:border-brand-500 dark:border-white/15 dark:bg-white/5">
                  ...
                  <input id="username" value={username} onChange={(event) => onUsernameChange(event.target.value)} placeholder="your-name" maxLength={30} aria-describedby="username-status" className="min-w-0 flex-1 bg-transparent py-3 text-sm outline-none" />
                ...
                <button type="button" onClick={getStarted} disabled={checking} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-neutral-950 px-6 text-sm font-semibold text-white disabled:opacity-60 dark:bg-white dark:text-neutral-950">Claim my URL <ArrowRight className="h-4 w-4" /></button>
              </div>
              <p id="username-status" aria-live="polite" className={`mt-2 min-h-5 text-center text-xs ${available ? 'text-emerald-600' : 'text-rose-600'}`}>
                {error || (available === true ? 'This address is available.' : '')}
              </p>
            </div>
          </Container>
        </section>
        ...
        <section id="features" className="py-16 sm:py-24">
          <Container>
            <div className="mx-auto max-w-2xl text-center"><p className="text-sm font-semibold uppercase tracking-[.16em] text-brand-600">The essentials, done right</p><h2 className="mt-3 text-3xl font-semibold sm:text-4xl">Turn your bio into a real showcase</h2><p className="mt-4 text-neutral-600 dark:text-white/60">A fast experience for visitors and practical tools for you.</p></div>
